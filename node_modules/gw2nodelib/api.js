...
var apiRequest = function(apiKey, options, callback, bypassCache) {
	...
	if (bypassCache || typeof cache.get(apiKey, cacheKey) === 'undefined' || (new Date()) > cache.get(apiKey, cacheKey).updateAt) {
		...
		request(url, function(error, response, body) {
			...
			} else callback(JSON.parse(body), headerSet);
		});
		return;
	}
...
module.exports = function() {
	...
	ret.load = function(apiKey, bypass) {
		ret.data[apiKey] = []; // fill data.apiKey with all possible data
		...
		var outerCallback = function(jsonList, headers) { //single fetch to get max pages
			if (jsonList.error) {
				...
				ret.data[apiKey] = ret.data[apiKey].concat(jsonList); //append first fetch to data.apiKey
				var num = 0;
				var total = headers.pageTotal - 1; //variable of max pages for looping
				var half_length = Math.ceil((headers.pageTotal - 1) / 2); //variable to identify half of max pages
				if (config.debug) console.log("half is " + half_length);
				var innerCallback = function(jsonList, headers) {
					if (jsonList.error) {
						ret.error(jsonList.error);
					} else {
						ret.data[apiKey] = ret.data[apiKey].concat(jsonList); //append data
					}
					num++; // track progress
					if (num == half_length) ret.half(apiKey); //call half callback at half
					if (num == total) ret.done(apiKey); //call done callback when done.
				};
				for (var nextPage = 1; nextPage < headers.pageTotal; nextPage++) { //LOOP over all pages. This is asych, so callback callbacks when done.
					var innerFetchParams = {
						page: nextPage,
						page_size: 200
					};
					//inner call
					ret[apiKey](innerCallback, innerFetchParams, bypass); //ENDLOOP headers go here.
				}
			}
		};
		//Call the ball
